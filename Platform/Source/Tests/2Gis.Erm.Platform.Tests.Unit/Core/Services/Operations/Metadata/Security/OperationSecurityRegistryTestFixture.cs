...
namespace DoubleGis.Erm.Platform.Tests.Unit.Core.Services.Operations.Metadata.Security
{
    ...
    public sealed class OperationSecurityRegistryTestFixture
    {
        ...
        public void AccessRequirement_OperationIdentity_MustBeCreatedAccordingToGenericTypes()
        {
            ...
            Assert.AreEqual(requirement.StrictOperationIdentity, new StrictOperationIdentity(CreateIdentity.Instance, new EntitySet(EntityName.Order)));
        }
        ...
        public void AccessRequirement_OperationUsages_MustBeCreatedAccordingToGenericTypes()
        {
            var requirement = AccessRequirementBuilder.ForOperation<CreateIdentity, Order>(
                x => x.UsesOperation<CalculateOrderReleaseWithdrawalsIdentity>()
            ...
            Assert.Contains(new StrictOperationIdentity(CalculateOrderReleaseWithdrawalsIdentity.Instance, EntitySet.Create.NonCoupled), usedOperations);
        ...
        {
            var requirement = AccessRequirementBuilder.ForOperation<CreateIdentity, Order>(
                x => x.Require(EntityAccessTypes.Create, EntityName.Order)
                      .Require(EntityAccessTypes.Read, EntityName.Firm)
                      .Require(EntityAccessTypes.Update, EntityName.File)
        ...
        public void AccessRequirementReader_MustDeriveRequirementsFromChildOperations()
        {
            ...
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Create, EntityName.Order), requirements, "Список должен содержать явно объявленную привелегию");
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Create, EntityName.Firm), requirements, "Список должен содержать выведенную из дочерней операции привелегию");
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Update, EntityName.Firm), requirements, "Список должен содержать выведенную из дочерней операции привелегию");
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Update, EntityName.Theme), requirements, "Список должен содержать выведенную из дочерней операции привелегию");
        }
        ...
        public void AccessRequirementReader_ParentOperationsMustNotInfluenceToChild()
        {
            ...
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Create, EntityName.Firm), requirementsForOperation2, "Список должен содержать привелегию");
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Update, EntityName.Firm), requirementsForOperation3, "Список должен содержать привелегию");
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Update, EntityName.Theme), requirementsForOperation3, "Список должен содержать привелегию");
            Assert.Contains(new EntityAccessRequirement(EntityAccessTypes.Update, EntityName.Theme), requirementsForOperation4, "Список должен содержать привелегию");
        }
        ...
        public void OperationDependencyResolver_ShouldReturnNullOnUnknownOperation()
        {
            ...
            var result = resolver.TryGetOperationRequirements(new StrictOperationIdentity(UpdateIdentity.Instance, new[] { EntityName.Firm }.ToEntitySet()), out notUsed);
