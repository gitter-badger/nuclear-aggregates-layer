...
Ext.ux.Calendar = Ext.extend(Ext.Component, {
    ...
    setValue: function (date, silent)
    {
        ...
        silent = silent || false;
        // TODO зачем-то затирается точное дата и время/вместо этого остается просто дата,
        // получаем возможность ошибки - получили с сервера дату со временем, добавили смещение, отрезали часть содержащую время,
        // перед постом карточки на сервер, убираем смещение из this.el.dom.value, однако в итоге получаем не ту же дату и время которую первоночально получили от сервера
        ...
        // Пока эта ошибка маскируется тем, что:
        // - либо дата время используется как просто дата => из-за усечения доп. погрешность не вносится
        // - либо данное значение даты времени присваивается где-то в серверной логике, и данные передаваемые с клиента игнорируются, т.е. для клиента эта дата и время readonly
        // Пока усечение оставлено, т.к. если его убрать начинает отображаться не только дата но и время в контроле календаря, однако, очевидно нужно изменить сам контрол,
