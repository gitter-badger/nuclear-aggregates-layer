...
{
    [Migration(6485, "Изменение логики загрузки карточек - теперь мы реплицируем данные чуть позже по расписанию")]
    public sealed class Migration6485 : TransactedMigration
    {
        #region Текст запроса
        private const string ScriptText = @"ALTER PROCEDURE [Integration].[ImportCardsFromXml]
    @Doc xml = NULL, 
    @ModifiedBy int = NULL,
    @OwnerCode int = NULL,
...
(
    ...
    (
       ...
       INSERT INTO Integration.FirmsForPostIntegrationActivities SELECT FirmId, GETUTCDATE(), 0, 0 FROM #XmlCardTbl
