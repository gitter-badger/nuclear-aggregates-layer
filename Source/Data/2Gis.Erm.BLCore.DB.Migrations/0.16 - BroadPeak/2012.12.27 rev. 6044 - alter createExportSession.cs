...
{
    [Migration(6044, "Изменение хранимой процедуры CreateExportSession")]
    public sealed class Migration6044 : TransactedMigration
    {
...
BEGIN
	INSERT INTO @ids
	SELECT TOP 300 * FROM
(SELECT DISTINCT(EntityId), NULL as timestampfield
	FROM 
	Integration.ExportSessionDetails	IESD
	JOIN Integration.ExportSessions IES ON IES.Id = IESD.IntegrationExportSessionId
	WHERE IES.EntityType = @entityTypeId
		AND (SELECT TOP 1 IsSuccessful FROM Integration.ExportSessionDetails IESD2 WHERE IESD2.EntityId = IESD.EntityId ORDER BY Id DESC) = 0) as NE
END
ELSE
BEGIN
