...
{
    [Migration(9356, "Изменяю хранимку ImportCardsFromXml, удаляю ExportOrdersAfterImportCards и таблицу FirmsForPostIntegrationActivities")]
    public class Migration9356 : TransactedMigration
    {
        #region AlterStatement
        private const string AlterStatement = @"
/****** Object:  StoredProcedure [Integration].[ImportCardsFromXml]    Script Date: 12.06.2013 13:38:14 ******/
SET ANSI_NULLS ON
GO
...
BEGIN TRY
    ...
    BEGIN TRAN
       ...
       INSERT INTO @TerritoriesTbl EXEC Shared.GetTemporaryTerritories @OrganizationUnits = @BranchCodes, @ModifiedBy = @ModifiedBy, @OwnerCode = @OwnerCode
...
"; 
        ...
        protected override void ApplyOverride(IMigrationContext context)
        {
            try
            {
                context.Connection.ExecuteNonQuery(AlterStatement);
            }
            catch (Exception)
            {
                // Хранимку не удаётся изменить в Main из-за удаления OwnerCode
                // Это не страшно, сделана миграция #9805, которая объединяет изменения этой хранимки в 0.18 и Main
            }


            var table = context.Database.Tables["FirmsForPostIntegrationActivities", "Integration"];
            if (table != null)
            {
                table.Drop();
            }

            var procedure = context.Database.StoredProcedures["ExportOrdersAfterImportCards", "Integration"];
            if (procedure != null)
            {
                procedure.Drop();
            }
