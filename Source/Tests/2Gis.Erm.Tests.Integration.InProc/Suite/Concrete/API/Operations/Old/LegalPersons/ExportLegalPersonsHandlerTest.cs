...
namespace DoubleGis.Erm.Tests.Integration.InProc.Suite.Concrete.API.Operations.Old.LegalPersons
{
    public class ExportLegalPersonsHandlerTest : UseModelEntityHandlerTestBase<OrganizationUnit, ExportLegalPersonsRequest, IntegrationResponse>
    {
        ...
        protected override OrdinaryTestResult AssertResponse(IntegrationResponse response)
        {
            return Result.When(response).Then(x => x.Stream.Should().NotBeNull());
        }
    }
}