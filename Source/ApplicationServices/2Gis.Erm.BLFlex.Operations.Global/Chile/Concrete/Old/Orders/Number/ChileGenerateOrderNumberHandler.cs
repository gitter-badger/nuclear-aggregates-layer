...
namespace DoubleGis.Erm.BLFlex.Operations.Global.Chile.Concrete.Old.Orders.Number
{
    public sealed class ChileGenerateOrderNumberHandler : RequestHandler<GenerateOrderNumberRequest, GenerateOrderNumberResponse>, IChileAdapted
    {
        private readonly IOrderReadModel _orderReadModel;
        private readonly OrderNumberGenerationStrategy[] _orderNumberGenerationStrategies;

        public ChileGenerateOrderNumberHandler(IOrderReadModel orderReadModel)
        {
        ...
        protected override GenerateOrderNumberResponse Handle(GenerateOrderNumberRequest request)
        {
            ...
            var numberFormat = string.Format("ORD_{0}-{1}-{2}", syncCodes[order.SourceOrganizationUnitId], syncCodes[order.DestOrganizationUnitId], "{0}");
