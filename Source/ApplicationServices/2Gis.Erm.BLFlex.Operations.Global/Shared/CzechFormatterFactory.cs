...
{
    // FIXME {all, 27.02.2014}: Является клоном FormatterFactory с адаптацией форматирования длинной даты для Чехии
    public sealed class CzechFormatterFactory : IFormatterFactory
    {
        ...
        private static readonly IDictionary<FormatType, IFormatter> Formatters = new Dictionary<FormatType, IFormatter>
            {
                { FormatType.LongDate, new StringFormatter("{0:d'. 'MMMM' 'yyyy}") }, 
                { FormatType.ShortDate, new StringFormatter("{0:d'. 'M'. 'yyyy}") }, 
