...
namespace DoubleGis.Erm.BLFlex.Aggregates.Global.Chile
{
    ...
    {
        public static IFindSpecification<DictionaryEntityInstance> FindOnlyBanks()
        {
            return new FindSpecification<DictionaryEntityInstance>(entity => entity.DictionaryEntityPropertyInstances.Any(property =>
                property.PropertyId == EntityTypeNameIdentity.Instance.Id && property.NumericValue == (int)EntityName.Bank));
        }

        public static ISelectSpecification<DictionaryEntityInstance, Bank> Select()
        {
