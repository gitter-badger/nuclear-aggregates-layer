                ...
                try
                {
                    while (true)
                    {
                        ...
                        if (package.Length != 0)
                        {
                            ...
                            long[] firmIdsFromParsedDto = !string.IsNullOrWhiteSpace(dto.FirmXml) ? GetFirmIdsFromParsedDto(dto) : new long[0];
                            ...
                            {
                                operationScope.Updated<Firm>(firmIdsFromParsedDto)
                                              .ApplyChanges<Firm>(importResult.ImportFirmsChanges, out mergedImportFirmChanges)
                                              .ApplyChanges<FirmAddress>(importResult.ImportFirmsChanges)
                                              .Complete();
                            }
                            ...
                            {
                                _logger.WarnFormatEx("There are different firm changes detected from parsed dto and reported by aggregate service, ids: " + string.Join(",", diffParsedDtoAndAggregateService));
                            }
                            ...
                            {
                                operationScope.ApplyChanges<Firm>(importResult.ImportCardsChanges)
                                              .ApplyChanges<FirmAddress>(importResult.ImportCardsChanges)
                                              .Complete();
                            }
                        }
