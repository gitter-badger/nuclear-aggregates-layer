...
namespace DoubleGis.Erm.BLCore.UI.Web.Mvc.Settings.ConfigurationDto
{
    public static class DtoUtils
    {
        ...
        /// <summary>
        /// Временное решение. Выполняет mapping настроек карточки из метаданных в специфичные структуры для web mvc клиента - необходимо для поддержки jsonignore и т.п. специфики
        ...
        /// </summary>
        public static CardJson ToCardJson(this CardStructure cardStructure)
        {
            var result = new CardJson();
            result.InjectFrom<CloneInjection>(cardStructure);
            return result;
        }
