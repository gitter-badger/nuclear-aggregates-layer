...
{
    public sealed class PrintFormsCustomization : IViewModelCustomization<ICustomizableOrderViewModel>, IRussiaAdapted
    {
        public void Customize(ICustomizableOrderViewModel viewModel, ModelStateDictionary modelState)
        {
            var isActionDisabledBasedOnWorkflowStepId = !viewModel.IsTerminated || !(viewModel.WorkflowStepId == (int)OrderState.OnTermination || viewModel.WorkflowStepId == (int)OrderState.Archive);
