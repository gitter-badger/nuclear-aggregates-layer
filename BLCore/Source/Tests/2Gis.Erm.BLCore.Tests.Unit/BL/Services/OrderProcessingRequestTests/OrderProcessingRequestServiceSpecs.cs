            ...
            Establish context = () =>
                {
                    ...
                    Mock.Get(finder)
                        .Setup(x => x.Find(Moq.It.IsAny<IFindSpecification<OrderProcessingRequest>>()))
                        .Returns<IFindSpecification<OrderProcessingRequest>>(x => storage.Where(x.Predicate.Compile()).AsQueryable());
