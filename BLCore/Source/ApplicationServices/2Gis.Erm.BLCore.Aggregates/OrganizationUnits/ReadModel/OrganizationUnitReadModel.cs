...
namespace DoubleGis.Erm.BLCore.Aggregates.OrganizationUnits.ReadModel
{
    public class OrganizationUnitReadModel : IOrganizationUnitReadModel
    {
        ...
        {
            return _finder.FindObsolete(Specs.Find.ById<OrganizationUnit>(organizationUnitId)).Single();
        }

        public string GetName(long organizationUnitId)
        {
            return _finder.FindObsolete(Specs.Find.ById<OrganizationUnit>(organizationUnitId)).Select(x => x.Name).Single();
        }

        public IReadOnlyDictionary<long, string> GetNames(IEnumerable<long> organizationUnitIds)
        {
        ...
        {
            var currencyId = _finder.Find(new FindSpecification<OrganizationUnit>(x => x.Id == organizationUnitId))
                                    .Map(q => q.Select(x => (long?)x.Country.CurrencyId))
                                    .One();
        ...
        {
            return _finder.FindObsolete(Specs.Find.ById<OrganizationUnit>(organizationUnitId)).Select(x => x.SyncCode1C).Single();
        }

        public IReadOnlyDictionary<int, long> GetOrganizationUnitIdsByDgppIds(IEnumerable<int> dgppIds)
        {
            // ReSharper disable once PossibleInvalidOperationException
            return _finder.Find(OrganizationUnitSpecs.OrganizationUnits.Find.ByDgppIds(dgppIds)).Map(x => x.DgppId.Value, x => x.Id);
        } 
    }
}