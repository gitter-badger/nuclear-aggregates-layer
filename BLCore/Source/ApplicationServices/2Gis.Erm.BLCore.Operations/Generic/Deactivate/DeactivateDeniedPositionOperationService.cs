...
namespace DoubleGis.Erm.BLCore.Operations.Generic.Deactivate
{
    public class DeactivateDeniedPositionOperationService : IDeactivateGenericEntityService<DeniedPosition>
    {
        ...
        public DeactivateDeniedPositionOperationService(IDeactivateDeniedPositionAggregateService deactivateDeniedPositionAggregateService,
        ...
        public DeactivateConfirmation Deactivate(long entityId, long ownerCode)
        {
            using (var scope = _operationScopeFactory.CreateSpecificFor<DeactivateIdentity, DeniedPosition>())
            {
                ...
                {
                    throw new InactivePriceModificationException(BLResources.CantDeactivateDeniedPositionWhenPriceIsDeactivated);
                }

                if (deniedPosition.IsSelfDenied())
                {
                    _deactivateDeniedPositionAggregateService.DeactivateSelfDenied(deniedPosition);
                }
                else
                {
                    var symmetricDeniedPosition = _getSymmetricDeniedPositionOperationService.GetWithObjectBindingTypeConsideration(deniedPosition);
