        ...
        private string ProcessValue(EntityName entityName, string propertyName, string value)
        {
            ...
            switch (propertyName)
            {
                ...
                case "ModifiedBy":
                    return _userIdentifierService.GetUserInfo(long.Parse(value)).DisplayName;
                case "WorkflowStepId":
                    return EnumUtils.ParseEnum<OrderState>(value).ToStringLocalized(EnumResources.ResourceManager, _userContext.Profile.UserLocaleInfo.UserCultureInfo);
                case "DealStage":
                    return EnumUtils.ParseEnum<DealStage>(value).ToStringLocalized(EnumResources.ResourceManager, _userContext.Profile.UserLocaleInfo.UserCultureInfo);
                case "Status":
                    if (entityName == EntityName.Limit)
                    {
                        LimitStatus status;
                        return EnumUtils.TryParseEnum(value, out status) ? status.ToStringLocalized(EnumResources.ResourceManager, CultureInfo.CurrentCulture) : value;
                    }
