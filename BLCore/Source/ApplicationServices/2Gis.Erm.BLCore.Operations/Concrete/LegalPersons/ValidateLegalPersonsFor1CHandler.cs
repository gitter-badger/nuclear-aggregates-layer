...
{
    public sealed class ValidateLegalPersonsFor1CHandler : RequestHandler<ValidateLegalPersonsFor1CRequest, ValidateLegalPersonsResponse>, IValidateLegalPersonsForExportOperationService
    {
        ...
        public ValidateLegalPersonsFor1CHandler(ILegalPersonReadModel legalPersonReadModel)
        {
        ...
        {
            var errors = Validate(request.Entities);

            return new ValidateLegalPersonsResponse
                       {
                           BlockingErrors = errors.Where(x => x.IsBlockingError).ToArray(),
                           NonBlockingErrors = errors.Where(x => !x.IsBlockingError).ToArray(),
                       };
        }
        ...
        private void ValidateSingleItem(ValidateLegalPersonRequestItem request, string[] nonUniqueCodes1C, ICollection<ErrorDto> blockingErrors, ICollection<ErrorDto> nonBlockingErrors)
        {
            ...
            var profilesInfo = _legalPersonReadModel.GetLegalPersonWithProfiles(request.Entity.Id);

            if (profilesInfo != null && !profilesInfo.LegalPersonHasProfiles)
            {
        ...
        public IEnumerable<ErrorDto> Validate(IEnumerable<ValidateLegalPersonRequestItem> legalPersonsToValidate)
        {
            ...
            var legalPersons = _legalPersonReadModel.GetLegalPersons(legalPersonsToValidate.Select(x => x.LegalPersonId).ToArray());
            
            var blockingErrors = new List<ErrorDto>();
            var nonBlockingErrors = new List<ErrorDto>();

            foreach (var item in legalPersonsToValidate)
            {
                if (item.Entity == null)
                {
                    item.Entity = legalPersons.SingleOrDefault(x => x.Id == item.LegalPersonId);
                    if (item.Entity == null)
                    {
                        throw new EntityNotFoundException(typeof(LegalPerson), item.LegalPersonId);
                    }
                }
            ...
            return blockingErrors.Concat(nonBlockingErrors);
        }
    }
}