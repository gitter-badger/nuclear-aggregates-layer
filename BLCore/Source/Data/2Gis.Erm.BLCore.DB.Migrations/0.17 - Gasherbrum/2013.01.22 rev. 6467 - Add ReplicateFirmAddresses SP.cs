...
{
    [Migration(6467, "Добавление процедуры репликации адресов фирм")]
    public class Migration6467 : TransactedMigration
    {
        protected override void ApplyOverride(IMigrationContext context)
        {
            #region Текст запроса
            const string Query = @"CREATE PROCEDURE [BusinessDirectory].[ReplicateFirmAddresses]
...
AS
	...
	BEGIN
		...
		WHERE [FA].[Id] in (SELECT Id FROM @Ids);
		...
		WHERE [CFA].[FirmAddressId] in (SELECT Id FROM @Ids);
	...
	BEGIN
		...
		FROM [DoubleGis_MSCRM].[dbo].[Dg_firmaddressBase] AS [CRMFA]
			INNER JOIN [BusinessDirectory].[FirmAddresses] AS [FA] ON [CRMFA].[Dg_firmaddressId] = [FA].[ReplicationCode] AND [FA].[Id] in (SELECT Id FROM @Ids);
