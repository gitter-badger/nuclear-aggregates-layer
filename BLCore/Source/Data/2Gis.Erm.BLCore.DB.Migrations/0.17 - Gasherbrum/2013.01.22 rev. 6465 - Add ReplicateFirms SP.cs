...
{
    [Migration(6465, "Добавление процедуры репликации фирм")]
    public class Migration6465 : TransactedMigration
    {
        protected override void ApplyOverride(IMigrationContext context)
        {
            #region Текст запроса
            const string Query = @"
CREATE PROCEDURE [BusinessDirectory].[ReplicateFirms]
...
AS
	...
	BEGIN
		...
		FROM [DoubleGis_MSCRM].[dbo].[Dg_firmBase] AS [CRMF]
			INNER JOIN [BusinessDirectory].[Firms] AS [F] ON [CRMF].[Dg_firmId] = [F].[ReplicationCode] AND [F].[Id] in (SELECT Id FROM @Ids);
