...
{
    [Migration(10758, "Заменяем вхождения Territories.DgppId на Territories.Id в процедурах GetTemporaryTerritories, UpdateBuildings")]
    public sealed class Migration10758 : TransactedMigration
    {
        #region Alter GetTemporaryTerritories

        private const string AlterGetTemporaryTerritories = @"
-- changes
...
BEGIN
	BEGIN TRY
		...
		INSERT INTO @TempTbl SELECT 
			Id, OrganizationUnitId, NULL FROM BusinessDirectory.Territories WHERE OrganizationUnitId IN (SELECT DISTINCT OrganizationUnitId FROM @OrganizationUnits) AND IsActive = 1 AND Name like '%' + @RegionalTerritoryLocalName + '%' ORDER BY Id DESC
...
";
        ...
        #region Alter UpdateBuildings

        private const string AlterUpdateBuildings = @"
-- changes
--   24.06.2013, a.rechkalov: замена int -> bigint
--   30.07.2013, a.tukaev: [ERM-387] заменил все вхождения Territories.DgppId на Territories.Id
...
";
        ...
        {
            context.Database.ExecuteNonQuery(AlterGetTemporaryTerritories);
            context.Database.ExecuteNonQuery(AlterUpdateBuildings);
        }
    }
}