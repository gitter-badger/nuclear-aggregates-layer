...
{
    [Migration(7778, "Преобразование полей Id в 64 разрядные")]
    public sealed class Migration7778 : IContextedMigration<IMigrationContext>
    {
        public void Apply(IMigrationContext context)
        {
            var clusters = Initialize(context).ToArray();

            var progress = new Progress(clusters.Length, context.Output);
        ...
        private static Cluster[] Initialize(IMigrationContext context)
        {
            ...
            var idsToProcess = context.Database.Tables.OfType<Table>()
                                      .Where(table => table.Name != "FileBinaries") // костыль: эту таблицу не надо явно апгрейдить, она сама...
                                      .Select(table => table.Columns["Id"])
                                      .Where(id => id != null && id.DataType.SqlDataType == DataType.Int.SqlDataType)
                                      .Where(IsNotForeignKey)
                                      .ToList();
        ...
        private static bool IsNotForeignKey(Column arg)
        {
            var table = (Table)arg.Parent;
            var anyForeignKey = table.ForeignKeys.Cast<ForeignKey>().Any(key => key.Columns.Cast<ForeignKeyColumn>().Any(column => column.Name == arg.Name));
            return !anyForeignKey;
        }
        ...
        private class Progress
        {
            ...
            public Progress(int count, TextWriter output)
            {
                _count = count;
                _output = output;
            ...
            public void Increment()
            {
                ...
                if (_displayedPosition + Step < percentComplete)
                {
                    ...
                    _output.WriteLine(percentComplete + "%");
                }
            }
        }
        ...
        private class DatabaseExplorer
        {
            ...
            public Cluster InitializeColumns(Column column)
            {
                ...
                var indexes = GetRelatedIndexes(columns)
                    .Concat(GetTableIndexes((Table)column.Parent))
                    .Distinct();
            ...
            private IEnumerable<Index> GetTableIndexes(Table parent)
            {
                // достаточно много времени потрачено, поэтому - костыль.
                return parent.Indexes.OfType<Index>().Where(index => index.Name == "IX_Territories_DgppId_NotNull");
            }
