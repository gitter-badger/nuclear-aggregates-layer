...
{
    [Migration(7831, "Преобразование несвязанных полей 64 разрядные")]
    public sealed class Migration7831 : IContextedMigration<IMigrationContext>
    {
        private static readonly ObjectName[] SingleColumns = new[]
            {
                new ObjectName { Name = "ClientId", TableName = "Limits", TableSchema = "Billing" },
                new ObjectName { Name = "PositionId", TableName = "DeniedPositions", TableSchema = "Billing" },
                new ObjectName { Name = "ParentId", TableName = "Notes", TableSchema = "Shared" },
                new ObjectName { Name = "OrderId", TableName = "ReleaseValidationResults", TableSchema = "Billing" },
                new ObjectName { Name = "OrderValidationGroupId", TableName = "OrderValidationResults", TableSchema = "Billing" },

                new ObjectName { Name = "InspectorCode", TableName = "Limits", TableSchema = "Billing" },
                new ObjectName { Name = "InspectorCode", TableName = "Orders", TableSchema = "Billing" },
            };

        public void Apply(IMigrationContext context)
        {
            var clusters = Initialize(context, SingleColumns).ToArray();
        ...
        private static Cluster[] Initialize(IMigrationContext context, ObjectName[] targets)
        {
            var explorer = new DatabaseExplorer(context.Database);

            var columns = context.Database.Tables.OfType<Table>()
                                      .SelectMany(table => table.Columns.OfType<Column>())
                                      .Where(column => column.DataType.SqlDataType == SqlDataType.Int)
                                      .Where(column => targets.Any(target => target.Name == column.Name
                                          && target.TableSchema == ((Table)column.Parent).Schema
                                          && target.TableName == ((Table)column.Parent).Name))
                                      .ToList();

            var clusters = new List<Cluster>();
            while (columns.Count > 0)
            {
                var id = columns.First();
                var cluster = explorer.InitializeColumns(id);
                clusters.Add(cluster);

                foreach (var column in cluster.ColumnNames)
                {
                    columns.RemoveAll(c => ((Table)c.Parent).Name == column.TableName
