...
{
    [Migration(12072, "Удаление дублей контактов фирмы и altert импорта карточек")]
    public class Migration12072 : TransactedMigration
    {
        private const string Query = @"WITH Doubles AS (
select Id, FirmAddressId, SortingPosition
, row_number() over(partition by FirmAddressId, SortingPosition order by Id) num
...
";
        ...
        protected override void ApplyOverride(IMigrationContext context)
        {
            ...
            AlterImportCards(context);
        }

        private void AlterImportCards(IMigrationContext context)
        {
...
-- 24.09.2013, y.baranihin: при импорте контактов должно изменяться поле CreatedOn
		...
		BEGIN TRAN
		   ...
		   WHEN MATCHED THEN 
				 UPDATE SET 
						...
						CurrentValues.ModifiedBy = @ModifiedBy,
						CurrentValues.ModifiedOn = GETUTCDATE(),
						CurrentValues.CreatedBy = @ModifiedBy,
						CurrentValues.CreatedOn = GETUTCDATE()
		   WHEN NOT MATCHED BY TARGET THEN 
...
";
            ...
            context.Database.ExecuteNonQuery(alterQuery);
        }
    }
}
