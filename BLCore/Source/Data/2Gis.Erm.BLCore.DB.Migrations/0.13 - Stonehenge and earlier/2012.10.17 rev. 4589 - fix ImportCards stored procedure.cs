...
{
    [Migration(4589, "Исправление импорта карточек")]
    public sealed class Migration4589 : TransactedMigration
    {
        #region Текст запроса
        private const String ScriptText = @"
IF EXISTS(SELECT name FROM sysobjects WHERE name = N'' + 'ImportDepCardsFromXml' AND xtype='P')
DROP PROCEDURE Integration.ImportDepCardsFromXml

IF TYPE_ID(N'[Integration].[CardsTableType]') IS NOT NULL
...
IF EXISTS(SELECT name FROM sysobjects WHERE name = N'' + 'ImportCardsFromXml' AND xtype='P')
DROP PROCEDURE Integration.ImportCardsFromXml
GO
...
Create PROCEDURE [Integration].[ImportCardsFromXml]
    @Doc xml = NULL, 
    @ModifiedBy int = NULL,
	@OwnerCode int = NULL,
	@EnableReplication bit = 1
...
(
	...
	inner join @TerritoriesTbl as Territories ON CardsTable.OrganizationUnitId = Territories.OrganizationUnitId WHERE CardsTable.BuildingCode NOT IN (SELECT DISTINCT Code FROM Integration.Buildings)
