...
{
    [Migration(3697, "Изменение хранимой процедуры CreateExportSession - добавлен экспорт рекламных материалов")]
    public sealed class Migration3697: TransactedMigration
    {
...
BEGIN
	...
	BEGIN
		...
		(			
		SELECT O.Id, (CASE WHEN OP.MaxPositionsTimestamp > O.Timestamp THEN OP.MaxPositionsTimestamp ELSE O.Timestamp END) MaxTimeStamp
	...
	BEGIN
		...
		SELECT TOP 1000 * FROM
		(
		SELECT A.Id, (CASE WHEN AE.MaxElementsTimestamp > A.Timestamp THEN AE.MaxElementsTimestamp ELSE A.Timestamp END) MaxTimeStamp
