	...
	IF NOT EXISTS (SELECT 1 FROM [DoubleGis{0}_MSCRM].[dbo].[Dg_orderBase] WHERE [Dg_orderId] = @CrmId)
	--------------------------------------------------------------------------
	-- there is no such a record in the CRM database => insert a brand new one
	--------------------------------------------------------------------------
	BEGIN
	
		INSERT INTO [DoubleGis{0}_MSCRM].[dbo].[Dg_orderBase]
           ([CreatedBy]
           ,[CreatedOn]
           ,[DeletionStateCode]
           ,[Dg_orderId]
		...
		INSERT INTO [DoubleGis{0}_MSCRM].[dbo].[Dg_orderExtensionBase]
           ([Dg_amounttowithdraw]
		   ,[Dg_amountwithdrawn]
		...
		SELECT
		   ...
		   ,[Orders].[HasDocumentsDebt]				-- <Dg_hasdebt, tinyint>
	...
	BEGIN
		...
		FROM [DoubleGis{0}_MSCRM].[dbo].[Dg_orderBase] AS [CRMO]
		...
		FROM [DoubleGis{0}_MSCRM].[dbo].[Dg_orderExtensionBase] AS [CRMO]
			INNER JOIN [Billing].[Orders] ON [CRMO].[Dg_orderId] = [Orders].[ReplicationCode] AND [Orders].[Id] = @Id
			JOIN [Billing].[OrderExtensions] ON [OrderExtensions].Id = [Orders].Id
	END;
