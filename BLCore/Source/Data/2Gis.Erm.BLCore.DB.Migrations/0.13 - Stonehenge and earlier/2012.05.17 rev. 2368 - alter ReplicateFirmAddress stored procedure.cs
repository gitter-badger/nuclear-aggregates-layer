...
{
	[Migration(2368, "Изменяем хранимую процедуру ReplicateFirmAddress")]
	public sealed class Migration2368 : TransactedMigration
	{
        protected override void ApplyOverride(IMigrationContext context)
        {
            const string storedProcedureName = "ReplicateFirmAddress";
            ...
            var spTextBody = string.Format(ReplicateFirmAddressSp, context.CrmDatabaseName);

            if (context.Database.StoredProcedures.Contains(storedProcedureName, ErmSchemas.BusinessDirectory))
            {
                var sp = context.Database.StoredProcedures[storedProcedureName, ErmSchemas.BusinessDirectory];
                sp.TextBody = spTextBody;
                sp.Alter();
            }
            else
            {
                var sp = new StoredProcedure(context.Database,
                                         storedProcedureName,
                                         ErmSchemas.BusinessDirectory)
                {
                    TextMode = false,
	    ...
	    private const string ReplicateFirmAddressSp = @"
	SET NOCOUNT ON;
