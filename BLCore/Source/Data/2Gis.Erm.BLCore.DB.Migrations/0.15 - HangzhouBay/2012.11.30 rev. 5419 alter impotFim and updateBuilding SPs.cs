...
{
    [Migration(5419, "Обновление процедур импорта фирмы и обновления здания")]
    public sealed class Migration5419 : TransactedMigration
    {
        protected override void ApplyOverride(IMigrationContext context)
        {
            ...
            var alterImportFirmQuery = @"ALTER PROCEDURE [Integration].[ImportFirmFromXml]
	@Xml [xml] = NULL,
	@ModifiedBy [int] = NULL,
	@OwnerCode [int] = NULL,
...
END CATCH";
            ...
            var alterUpdateBuilding = @"ALTER PROCEDURE [Integration].[UpdateBuildings]
       @buildingsXml [xml]
AS
...
BEGIN TRAN
	...
	DECLARE @FirstActiveAdressForFirm TABLE(SortingPosition int NOT NULL, FirmId int NOT NULL)
	INSERT INTO @FirstActiveAdressForFirm
	SELECT min(AFA.SortingPosition), AFA.FirmId FROM @buildings buildings
	INNER JOIN BusinessDirectory.FirmAddresses FA ON FA.BuildingCode = buildings.Code
	INNER JOIN BusinessDirectory.FirmAddresses AFA ON FA.FirmId = AFA.FirmId AND AFA.IsActive = 1
...
END CATCH";
            ...
            context.Database.ExecuteNonQuery(alterImportFirmQuery);
            context.Database.ExecuteNonQuery(alterUpdateBuilding);
        }
    }
}
