CREATE PROCEDURE [Activity].[ReplicatePhonecalls]
...
AS
    ...
    BEGIN TRY
        ...
        USING (
            ...
            JOIN [Security].[Users] [modifiers] ON [modifiers].[Id] = [phonecalls].[ModifiedBy]
            ...
            WHERE [phonecalls].[Id] IN (SELECT [Id] FROM @Ids)
        ) AS [Modified]
		...
		ON [ActivityId] = [phonecalls].[ReplicationCode] AND [phonecalls].[Id] IN (SELECT [Id] FROM @Ids);
