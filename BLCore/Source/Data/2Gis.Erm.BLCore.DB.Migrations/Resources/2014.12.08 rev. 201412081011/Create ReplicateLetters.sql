CREATE PROCEDURE [Activity].[ReplicateLetters]
...
AS
    ...
    BEGIN TRY
        ...
        USING (
            ...
            WHERE [letters].[Id] IN (SELECT [Id] FROM @Ids)
        ) AS [Modified]
		...
		ON [ActivityId] = [letters].[ReplicationCode] AND [letters].[Id] IN (SELECT [Id] FROM @Ids);
