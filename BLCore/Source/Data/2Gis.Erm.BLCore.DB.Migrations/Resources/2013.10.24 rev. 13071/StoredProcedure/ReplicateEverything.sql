...
--   24.10.2013, a.rechkalov: Добавил вызов ReplicateOrderProcessingRequest
...
--    BEGIN TRANSACTION TxReplicateEverything
   ...
   DECLARE firmCursor CURSOR LOCAL FORWARD_ONLY STATIC FOR 
    ...
    DECLARE orderProcessingRequestCursor CURSOR LOCAL FORWARD_ONLY STATIC FOR 
			SELECT Id from Billing.OrderProcessingRequests ERM_OPR
			LEFT JOIN [DoubleGis_MSCRM].[dbo].[Dg_orderprocessingrequestBase] CRM_OPR ON ERM_OPR.ReplicationCode=CRM_OPR.[Dg_orderprocessingrequestId]
			WHERE CRM_OPR.ModifiedOn IS NULL OR ERM_OPR.ModifiedOn != CRM_OPR.ModifiedOn
    OPEN orderProcessingRequestCursor;
    WHILE 1 = 1
    BEGIN
          FETCH NEXT FROM orderProcessingRequestCursor INTO @id;
          IF @@FETCH_STATUS <> 0 BREAK;
          PRINT N'Репликация заявки с идентификатором '+ CONVERT(varchar(25), @id)
          EXECUTE [Billing].[ReplicateOrderProcessingRequest] @id;
          SET @counter = @counter + 1;
    END
    CLOSE orderProcessingRequestCursor;
    DEALLOCATE orderProcessingRequestCursor;
