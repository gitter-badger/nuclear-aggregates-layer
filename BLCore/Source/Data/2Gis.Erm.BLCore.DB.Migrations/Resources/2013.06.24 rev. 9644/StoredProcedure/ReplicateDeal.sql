...
AS
	...
	BEGIN
		...
		FROM [DoubleGis_MSCRM].[dbo].[OpportunityExtensionBase] AS [CRMD]
			INNER JOIN [Billing].[Deals] AS [D] ON [CRMD].[OpportunityId] = [D].[ReplicationCode] AND [D].[Id] = @Id
	END;
	...
	BEGIN
		...
		-- OpportunityCloseBase
		UPDATE [DoubleGis_MSCRM].[dbo].[OpportunityCloseBase]
		   SET [ActualRevenue] = [D].[ActualProfit]
		FROM [DoubleGis_MSCRM].[dbo].[OpportunityCloseBase] AS [OC]
			INNER JOIN [DoubleGis_MSCRM].[dbo].[ActivityPointerBase] as [AP] ON [AP].[ActivityId] = [OC].[ActivityId] AND [AP].[RegardingObjectTypeCode] = 3
