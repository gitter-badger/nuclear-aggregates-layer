...
CREATE PROCEDURE [Shared].[GetTemporaryTerritories](
...
BEGIN
	BEGIN TRY
		...
		INSERT INTO @TempTbl SELECT 
			Id, OrganizationUnitId, NULL FROM BusinessDirectory.Territories WHERE OrganizationUnitId IN (SELECT DISTINCT OrganizationUnitId FROM @OrganizationUnits) AND IsActive = 1 AND Name like '%' + @RegionalTerritoryLocalName + '%' ORDER BY DgppId DESC
