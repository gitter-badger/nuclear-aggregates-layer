...
BEGIN
	...
	IF Object_Id(N'tempdb..#XmlCardTbl') IS NOT NULL DROP TABLE #XmlCardTbl
	IF Object_Id(N'tempdb..#XmlContactsTbl') IS NOT NULL DROP TABLE #XmlContactsTbl
	IF Object_Id(N'tempdb..#SchedulesTbl') IS NOT NULL DROP TABLE #SchedulesTbl
	...
	BEGIN TRY
		...
		BEGIN TRAN
		   ...
		   SELECT 
				 ...
				 NULL,
				 ISNULL(IsLocal, 1)
		   FROM OPENXML (@idoc, N'/Root/Card',1)
				WITH (
						CardCode BIGINT N'@Code',
						FirmCode BIGINT N'@FirmCode', 
						BranchCode BIGINT N'@BranchCode',
						CardType nvarchar(3) N'@Type',
						IsCardHidden BIT N'@IsHidden',
						IsCardArchived  BIT N'@IsArchived',
						IsCardDeleted  BIT N'@IsDeleted',
						BuildingCode BIGINT N'./Address/@BuildingCode',
						AddressCode BIGINT N'./Address/@AddressCode',
						Address nvarchar(max) N'./Address/@Text',
						PromotionalReferencePoint nvarchar(max) N'./Address/@PromotionalReferencePoint',
						ReferencePoint nvarchar(max) N'./Address/@ReferencePoint',
						IsLocal BIT N'@IsLocal')
		   ...
		   FROM OPENXML (@idoc, N'/Root/Card/Contacts/*',1)
				WITH (ContactName  nvarchar(250) N'@mp:localname',
		   ...
		   INSERT INTO @CategoriesDgppIdsTbl SELECT
				 ...
				 ISNULL(IsPrimary, 0),
				 NULL
		   FROM OPENXML (@idoc, N'/Root/Card/Rubrics/Rubric',1)
				WITH (
						CardCode BIGINT N'../../@Code', 
						 RubricCode INT N'@Code',
						 IsPrimary BIT N'@IsPrimary')
		   ...
		   INSERT INTO #SchedulesTbl SELECT
				 ...
				 0,
				 NULL
		   FROM OPENXML (@idoc, N'/Root/Card/Schedules/Schedule/*',1)
				WITH (
						CardCode BIGINT N'../../../@Code', 
						DayLabel NVARCHAR(3) N'@Label',
						DayFrom Time N'@From', 
						DayTo Time N'@To',
						BreakFrom Time N'./*/@From',
						BreakTo Time N'./*/@To')
		   ...
		   INSERT INTO  @PaymentTypesTbl SELECT 
				 ...
				 0
		   FROM OPENXML (@idoc, N'/Root/Card/Fields/ReferenceListField[@Code=''PaymentMethod'']/Items/*',1)
				 WITH (
				 CardCode BIGINT N'../../../../@Code',
				 ItemCode INT N'@Code') as XmlValues left join Integration.ReferenceItems On XmlValues.ItemCode = Integration.ReferenceItems.Code AND Integration.ReferenceItems.IsDeleted = 0 
				  AND Integration.ReferenceItems.ReferenceId IN (SELECT Id FROM Integration.Reference WHERE CodeName = N'PaymentMethod')
		   ...
		   UPDATE @XmlContactDtos set ContactTypeId = 2 Where ContactTypeId = 1 AND CanReceiveFax = N'True' -- телефон с функцией факса - это факс в понимании ERM
		   ...
		   BEGIN
			...
			if @CurrentPregeneratedId IS NULL 
			 begin
			   RAISERROR(N'Сгенерировано недостаточно идентификаторов', 16, 2)
			end
		   ...
		   SELECT * FROM #XmlCardTbl WHERE CardType = N'DEP'
		   ...
		   DELETE FROM #XmlCardTbl WHERE CardType = N'DEP'
		   ...
		   BEGIN
				 ...
				 BEGIN
						SET @CurrentPaymentMethod = @CurrentPaymentMethod + (SELECT LTRIM(STR(SortingPosition))+N'. '+Name+N' ' FROM @PaymentTypesTbl WHERE InsertOrder = @CurrentPaymentId)
		   ...
		   BEGIN
				 ...
				 SELECT FirmCode, N'Пустая фирма #' + LTRIM(STR(FirmCode, 20)),TerritoryId, OrganizationUnitId, 0, 0, 0, @OwnerCode, 0, 1, NEWID(), @ModifiedBy, GETUTCDATE(), @ModifiedBy, GETUTCDATE()
