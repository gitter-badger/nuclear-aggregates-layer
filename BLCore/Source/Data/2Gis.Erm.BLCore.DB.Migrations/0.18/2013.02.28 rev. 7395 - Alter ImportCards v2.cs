...
{
    // Миграция 7294 была создана в Main. Хранимка в ней не знала про существование колонки SyncFirmAddresses.
    // Колонка SyncFirmAddresses приехала позже из ветки, но там она имела номер миграции, меньший 7294.
    // В результате миграция 7294 не накатывается, а если она была накачена ранее 
    // (у тех программистов, что активно работали с main, либо на тестовых средах) хранимка из неё выполняться не будет.
    // Поэтому миграция 7294 очищена, вместо неё создана эта с учётом существования колонки SyncFirmAddresses.
    [Migration(7395, "Осмысленное сообщение при незнакомом коде ДГПП для подразделения")]
    public sealed class Migration7395 : TransactedMigration
    {
...
(
    ...
    (
       ...
       INSERT INTO Integration.FirmsForPostIntegrationActivities(FirmId, CreatedOn, ExportOrders, ReplicateObjects, SyncFirmAddresses)
	   SELECT Id, GETUTCDATE(), 0, 0, 0 FROM @FirmIds
    COMMIT TRAN
END TRY
BEGIN CATCH
       IF (XACT_STATE() != 0)
